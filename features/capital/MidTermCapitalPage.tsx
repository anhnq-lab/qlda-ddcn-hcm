    ...
    return (
        <div className="min-h-screen space-y-5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl flex items-center gap-3" style={{ fontFamily: 'var(--font-heading)' }}>
                        <div className="p-2.5 rounded-xl shadow-lg bg-gradient-to-br from-amber-500 to-yellow-600">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: 'VỐN TRUNG HẠN', value: fmtB(totalMidTerm), sub: `${midTermPlans.length} KH`, icon: <Landmark className="w-5 h-5" />, variant: 'stat-card-blue' },
                    { label: `VỐN GIAO ${yearFilter}`, value: fmtB(totalAnnual), sub: `${annualPlans.filter((p:any) => p.year === yearFilter).length} DA`, icon: <Calendar className="w-5 h-5" />, variant: 'stat-card-emerald' },
                    { label: `ĐÃ GIẢI NGÂN ${yearFilter}`, value: fmtB(totalDisbursed), sub: `${disbRate.toLocaleString('vi-VN', { maximumFractionDigits: 1 })}%`, icon: <DollarSign className="w-5 h-5" />, variant: 'stat-card-amber' },
                    { label: 'CHÊNH LỆCH', value: fmtB(totalAnnual - totalDisbursed), sub: disbRate < 50 ? 'Cần đẩy nhanh' : 'Bình thường', icon: <ArrowUpDown className={`w-5 h-5 ${disbRate < 50 ? 'animate-pulse' : ''}`} />, variant: disbRate < 50 ? 'stat-card-rose' : 'stat-card-violet' },
                ].map(kpi => (
                    <div key={kpi.label} className={`stat-card ${kpi.variant}`}>
                        <div className="stat-card-icon">{kpi.icon}</div>
                        <div>
                            <p className="stat-card-label">{kpi.label}</p>
                            <p className="stat-card-value">{kpi.value}</p>
                            <p className="text-[10px] font-bold mt-0.5" style={{ color: 'rgba(255,255,255,0.55)' }}>{kpi.sub}</p>
                        </div>
                    </div>
                ))}
            </div>
                        ...
                        {grpPlans.map((plan: any) => {
                            ...
                            return (
                                <div key={plan.plan_id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
                                    ...
                                    {isExp && (
                                        <div className="px-5 py-4 border-t border-gray-200 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-800/50">
                                            <div className="grid grid-cols-4 gap-3 mb-4">
                                                {[
                                                    ...
                                                    { label: 'Đã phân bổ HN', value: formatCurrency(linked.reduce((s:number, a:any) => s + Number(a.amount), 0)), cls: 'bg-amber-50 dark:bg-amber-900/20 text-amber-600' },
                                            ...
                                            {plan.notes && <div className="bg-white dark:bg-slate-700/50 p-2.5 rounded-lg mb-3 text-xs text-gray-600 dark:text-slate-300 italic border border-gray-100 dark:border-slate-600 flex items-start gap-1.5"><FileText className="w-3.5 h-3.5 mt-0.5 shrink-0 text-gray-400" /><span>{plan.notes}</span></div>}
                                            {linked.length > 0 && (
                                                <>
                                                    ...
                                                    <table className="w-full text-xs mb-3">
                                                        <thead className="bg-slate-100 dark:bg-slate-800 text-[10px] text-slate-500 font-bold uppercase border-b border-slate-200 dark:border-slate-700">
                                            ...
                                            <div className="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-slate-700">
                                                ...
                                                <button onClick={() => navigate(`/projects/${plan.project_id}`)} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-bold rounded-lg flex items-center gap-1 transition-all"><ArrowRight className="w-3 h-3" /> Xem dự án</button>
                                            </div>
                                        </div>
                                    )}
                                </div>
        ...
        <div className="section-card">
            <div className="overflow-x-auto">
                <table className="w-full text-xs" style={{ fontVariantNumeric: 'tabular-nums' }}>
                    <thead>
                        <tr className="table-header-row text-[10px] font-bold uppercase tracking-wider">
                ...
                <table className="w-full text-[11px] border-collapse" style={{ fontVariantNumeric: 'tabular-nums' }}>
                    <thead>
                        <tr className="table-header-row">
                            ...
                            {MONTHS.map((m, i) => <th key={m} className={`px-2 py-2 text-right text-[10px] font-bold uppercase min-w-[80px] ${i + 1 === currentMonth ? 'bg-amber-100/60 dark:bg-amber-900/20' : ''}`}>{m}</th>)}
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                        {projectIds.map((pid) => {
                            ...
                            return (
                                <tr key={pid} className="hover:bg-amber-50/20 dark:hover:bg-amber-900/5 transition-colors">
                                    ...
                                    {MONTHS.map((_, i) => {
                                        ...
                                        return <td key={i} className={`px-2 py-2 text-right font-mono text-slate-600 dark:text-slate-300 ${i + 1 === currentMonth ? 'bg-amber-50/40 dark:bg-amber-900/10' : ''}`}>{v > 0 ? fmtB(v) : ''}</td>;
                                    })}
                                    <td className="px-3 py-2 text-right font-mono font-bold text-amber-600 dark:text-amber-300">{totalPlanned > 0 ? fmtB(totalPlanned) : '—'}</td>
                                </tr>
                            );
                        })}
                    </tbody>
                    <tfoot className="bg-amber-50/30 dark:bg-amber-900/10 font-bold border-t-2 border-amber-200 dark:border-amber-800">
                        <tr>
                            <td className="px-3 py-2 sticky left-0 bg-amber-50/30 dark:bg-amber-900/10 z-10 font-black text-gray-800 dark:text-slate-100">TỔNG</td>
                            ...
                            {MONTHS.map((_, i) => {
                                ...
                                return <td key={i} className={`px-2 py-2 text-right font-mono text-amber-600 dark:text-amber-300 ${i + 1 === currentMonth ? 'bg-amber-100/60 dark:bg-amber-900/20' : ''}`}>{total > 0 ? fmtB(total) : ''}</td>;
                            })}
                            <td className="px-3 py-2 text-right font-mono text-amber-600 dark:text-amber-300 font-black">{fmtB(yearDisbPlans.reduce((s:number, d:any) => s + (Number(d.planned_amount) || 0), 0))}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
...
const DisbProgressTab: React.FC<{disbPlans: any[]; disbursements: any[]; annualPlans: any[]; yearFilter: number; searchTerm: string}> = ({ disbPlans, disbursements, annualPlans, yearFilter, searchTerm }) => {
    ...
    return (
        <div className="section-card">
            ...
            {/* Legend */}
            <div className="px-4 py-2 bg-gray-50 dark:bg-slate-700/50 border-t border-gray-200 dark:border-slate-700 flex items-center gap-4 text-[10px] text-gray-500">
